...
class MainCOBoilerContoller extends Controller
{
    // === coboiler
    public function index_coboiler()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['BOILER','COMMON'])->get();
        $weekly_data = CoBoiler::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.coboiler.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $coboiler_id = CoBoiler::where('id', $id)->first();
        return response()->json($coboiler_id);
    }
   ...
   {
       $today = Carbon::now()->format('d-m-Y');
       $new_data = coboiler::whereDate('updated_at', $today)->get();
       $user = User::where('id', Auth::user()->id)->first();
       $code = coboiler::where('status_equipment_id', 6)->orderBy('updated_at','desc')->get(); 
       return view('pages.admin.laporan.maintenance.coboiler.create', compact('user', 'code','today','new_data'));
   }
