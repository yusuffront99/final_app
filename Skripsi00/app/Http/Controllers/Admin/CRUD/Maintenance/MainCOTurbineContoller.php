...
class MainCOTurbineContoller extends Controller
{
    // === coturbine
    public function index_coturbine()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['TURBINE','COMMON'])->get();
        $weekly_data = CoTurbine::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.coturbine.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $coturbine_id = CoTurbine::where('id', $id)->first();
        return response()->json($coturbine_id);
    }
   ...
   {
       $today = Carbon::now()->format('d-m-Y');
       $new_data = coturbine::whereDate('updated_at', $today)->get();
       $user = User::where('id', Auth::user()->id)->first();
       $code = coturbine::where('status_equipment_id', 6)->orderBy('updated_at','desc')->get(); 
       return view('pages.admin.laporan.maintenance.coturbine.create', compact('user', 'code','today','new_data'));
   }
