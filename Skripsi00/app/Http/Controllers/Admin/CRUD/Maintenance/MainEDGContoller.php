...
class MainEDGContoller extends Controller
{
    // === EdgSystem
    public function index_edg()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['TURBINE','COMMON'])->get();
        $weekly_data = EdgSystem::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.edg.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $edg_id = EdgSystem::where('id', $id)->first();
        return response()->json($edg_id);
    }
