...
class MainFWPumpContoller extends Controller
{
    // === fwpump
    public function index_fwpump()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['BOILER','COMMON'])->get();
        $weekly_data = Fw_Pump::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.fwpump.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $fwpump_id = Fw_Pump::where('id', $id)->first();
        return response()->json($fwpump_id);
    }
   ...
   {
       $today = Carbon::now()->format('d-m-Y');
       $new_data = Fw_Pump::whereDate('updated_at', $today)->get();
       $user = User::where('id', Auth::user()->id)->first();
       $code = Fw_Pump::where('status_equipment_id', 6)->orderBy('updated_at','desc')->get(); 
       return view('pages.admin.laporan.maintenance.fwpump.create', compact('user', 'code','today','new_data'));
   }
