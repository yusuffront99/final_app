...
class MainHPPumpContoller extends Controller
{
    // === hppump
    public function index_hppump()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['BOILER','COMMON'])->get();
        $weekly_data = Hp_Pump::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.hppump.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $hppump_id = Hp_Pump::where('id', $id)->first();
        return response()->json($hppump_id);
    }
   ...
   {
       $today = Carbon::now()->format('d-m-Y');
       $new_data = Hp_Pump::whereDate('updated_at', $today)->get();
       $user = User::where('id', Auth::user()->id)->first();
       $code = Hp_Pump::where('status_equipment_id', 6)->orderBy('updated_at','desc')->get(); 
       return view('pages.admin.laporan.maintenance.hppump.create', compact('user', 'code','today','new_data'));
   }
