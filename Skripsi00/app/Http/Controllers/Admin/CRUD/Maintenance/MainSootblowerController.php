...
class MainSootblowerController extends Controller
{
    // === SOOTBLOWER
    public function index_sootblower()
    {
        $today = Carbon::now()->format('d-m-Y');
        $startDate = Carbon::now()->subWeek(); 
        $endDate = Carbon::now();
        $spare_part = SparePart::whereIn('category', ['BOILER','COMMON'])->get();
        $weekly_data = Sootblower::where('status_equipment_id', 6)->orderBy('updated_at','desc')->whereBetween('updated_at', [$startDate, $endDate])->get();

        return view('pages.admin.laporan.maintenance.sootblower.index', compact('today','weekly_data','spare_part'));
    }

    public function create_detail($id)
    {
        $sootblower_id = Sootblower::where('id', $id)->first();
        return response()->json($sootblower_id);
    }
   ...
   {
       $today = Carbon::now()->format('d-m-Y');
       $new_data = Sootblower::whereDate('updated_at', $today)->get();
       $user = User::where('id', Auth::user()->id)->first();
       $code = Sootblower::where('status_equipment_id', 6)->orderBy('updated_at','desc')->get(); 
       return view('pages.admin.laporan.maintenance.sootblower.create', compact('user', 'code','today','new_data'));
   }
